...
int userId = loggedInUser != null ? loggedInUser.getUserID() : -1; // 로그인한 사용자 ID, 미로그인시 -1

ProductMgr productMgr = new ProductMgr();
List<ProductBean> failedBidList = new ArrayList<>();

if(userId != -1) {
    failedBidList = productMgr.getFailedAuctions(userId); // 메서드 이름을 변경합니다.
}
%>
<!DOCTYPE html>
<html lang="en">
...
<li id="profileDeleteLi"><a href="./profileDelete.jsp">회원탈퇴신청</a></li>
    ...
    <table class="table">
        ...
        <tbody>
    <% int count = 0; // 리스트 아이템 카운트
       for(ProductBean product : failedBidList) {
           count++;
    %>
        <tr>
            <td><%= count %></td>
            ...
            <td><%= product.getAuction().getFinalPrice() %></td>
            <td><%= product.getCurrent().getWaybillNum() %></td>
        </tr>
    <% }
       if (count == 0) {
    %>
        <tr>
            <td colspan="5">유찰된 입찰이 없습니다.</td>
        </tr>
    <% } %>
</tbody>
