...
<body>
	...
	<div id="wrapper" class="container">
		...
		<section class="homepage-slider" id="home-slider">
			...
			List<ProductBean> productsForHomeSlider2 = productMgr.getProductsByBrand("Rolex", 3, auctionStatus);
		    pageContext.setAttribute("productsForHomeSlider2", productsForHomeSlider2);
			List<ProductBean> productsForCarousel = productMgr.getProductsByStatus(auctionStatus, 5); // 5개의 데이터를 가져옴
			...
			ProductBean randomMaleProduct = productMgr.getRandomMaleProduct("Female"); // 남성 상품 1개 랜덤하게 가져오기
			pageContext.setAttribute("randomMaleProduct", randomMaleProduct);
			List<ProductBean> impendingMaleAuctions = productMgr.getImpendingMaleAuctions();//마감임박 상품 가져오기
			%>
    ...
    <% if (randomMaleProduct != null) { %>
        <p><%=randomMaleProduct.getDescription()%></p>
    <% } %>
</section>
						...
						<div class="span12">
							<h4 class="title">
								<span class="pull-left"><span class="text"><span
										class="line">마감임박 <strong>Auction <i class="fa-solid fa-clock"></i></strong></span></span></span> <span
    ...
    <div class="carousel-inner">
        ...
        int itemCounter = 0;
        for (ProductBean product : impendingMaleAuctions) {
            // 4개의 상품마다 새로운 슬라이드 아이템 시작
            if (itemCounter % 4 == 0) {
                if (isFirstItem) {
                    out.print("<div class=\"item active\"><ul class=\"thumbnails\">");
                    isFirstItem = false;
                } else {
                    out.print("</ul></div>"); // Close previous item
                    out.print("<div class=\"item\"><ul class=\"thumbnails\">");
                }
            }
            // 상품 정보 출력
            %>
            ...
            <%
            itemCounter++;
            // 마지막 상품이거나 4의 배수가 되면 닫는 태그를 추가합니다
            if (itemCounter % 4 == 0 || itemCounter == impendingMaleAuctions.size()) {
                out.print("</ul></div>"); // Close current item
            }
        }
        if (itemCounter % 4 != 0) { // Ensure the last div is closed if not already
            out.print("</ul></div>");
        }
        %>
    </div>
    ...
    <div class="flexslider">
        <ul class="slides">
            <%
            // Rolex 브랜드 상품만 가져오는 메서드 호출
        
            for (ProductBean product : productsForHomeSlider2) {
            %>
            <li><img src="<%=product.getPhoto()%>" alt="<%=product.getName()%>" />
                <div class="intro">
                    <div class="product-info">
                        <h1>여자는 Louis Vuitton</h1>
                        <p>
                        </p>
                        <p>
...
<section>
				<div class="row">
					<div class="span12">
						<h4 class="title">
							<span class="pull-left"><span class="text"><span
									class="line">Woman<strong> popularity <i class="fa-solid fa-fire"></i> </strong></span></span></span> <span
								class="pull-right"> <a class="left button"
				...
				%><div
					class="item <%=(itemCounter3 == 0) ? "active" : ""%>">
					<ul class="thumbnails">
						<%
						}
