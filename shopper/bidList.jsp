    ...
    <div class="col">
        <p><span class="icon-text"><i class="fas fa-map-marker-alt"></i></span>배송지 정보: 
            <span id="currentAddress">
                <%= loggedInUser.getAddress() + " " + loggedInUser.getDetailAddress() %>
            </span> 
...
<div class="container content-box">
    ...
    <table class="table">
        ...
        <%   
          ...
          int pageSize = 10;
               ...
               Vector<MyPageBean> bidList = auctionMgr.bidList(user_id);
               // 전체 상품 개수를 가져옵니다.
              int totalProducts = bidList.size();
    ...
    <% for (int i = startIdx; i < endIdx; i++) { 
        MyPageBean bean = bidList.get(i);
        int maxBill = auctionMgr.maxWaybill(bean.getProductID());
    %>
    <tr <% if (maxBill != bean.getWaybillNum()) { %> style="background-color: rgba(255, 99, 71, 0.5)" <% } %>>
        <td><%= i+1 %></td>
        <td><%= bean.getProductID() %></td>
        <td>
            <a href="product_detail.jsp?id=<%= bean.getProductID() %>" style="text-decoration: none; color: black; cursor: pointer;">
                <%= bean.getName() %>
            </a>
        </td>
        <td><%= maxBill %></td>
        <td><%= bean.getWaybillNum() %></td>
    </tr>
    <% } %>
    ...
    <div class="text-center">
        <ul class="pagination pagination-sm m-0 ">
                         ...
                         <li class="page-item <%= pageNo == 1 ? "disabled" : "" %>">
                             <a class="page-link" href="bidList.jsp?pageNo=<%= pageNo - 1 %>">이전</a>
                         </li>
                         <!-- 페이지 번호 링크 -->
                         <% for (int i = 1; i <= totalPages; i++) { %>
                             <li class="page-item <%= i == pageNo ? "active" : "" %>">
                                 <a class="page-link" href="bidList.jsp?pageNo=<%= i %>"><%= i %></a>
                             </li>
                         <% } %>
                         <!-- 다음 페이지 링크 -->
                         <li class="page-item <%= pageNo == totalPages ? "disabled" : "" %>">
                             <a class="page-link" href="bidList.jsp?pageNo=<%= pageNo + 1 %>">다음</a>
                         </li>
                     </ul>
    </div>
</div>
    ...
    <script>
        ...
        $(document).ready(function() {
            // 임시 데이터
            var bidData = [
                { number: 1, id: "17 (abcd1234)", product: "슈프림 x 제이콥앤코 로고 링크 브레이슬릿 14k 골드 (22SS)", currentBid: "4,000,000원", myBid: "4,000,000원" },
                { number: 2, id: "19 (abcd1234)", product: "삼성 갤럭시 Z 폴드5 톰브라운 에디션 (국내 정식 발매 제품)", currentBid: "3,518,000원", myBid: "3,518,000원" },
                { number: 3, id: "15 (abcd1234)", product: "포켓몬 카드 게임 25주년 울트라 프리미엄 메탈 피카츄 (북미판)", currentBid: "125,000원", myBid: "125,000원" },
                { number: 4, id: "20 (abcd1234)", product: "카우스 패밀리 바이닐 피규어 세트", currentBid: "3,000,000원", myBid: "3,000,000원" },
                { number: 5, id: "18 (abcd1234)", product: "에르메스 피코탄 락 럭키 데이지 18 백 프린티드 스위프트 &...", currentBid: "5,500,000원", myBid: "5,500,000원" },
                { number: 6, id: "6 (abcd1234)", product: "구찌 마몬트 남성 반지갑", currentBid: "420,000원", myBid: "420,000원" }
            ];
