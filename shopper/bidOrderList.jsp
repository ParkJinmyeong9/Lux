    ...
    <script>
     ...
     // JavaScript 코드
        $(document).ready(function() {
            // 임시 데이터
            var bidData = [
                { number: 1, id: "10 (abcd1234)", product: "에르메스 집시에르 미니백 에버컬러 & 골드 하드웨어 골드", status: "<a href='http://nplus.doortodoor.co.kr/web/detail.jsp?slipno=587931883853' target='_blank' class='check-status' data-tracking='587931883853'>상태 확인</a>", myBid: "11,500,000원", review: "<a href='review.jsp?product_id=12345' class='btn btn-primary'>리뷰 작성</a>"},
                { number: 2, id: "11 (efgh5678)", product: "샤넬 클래식 퀼팅 미디 플랩백 블랙 램스킨 SHW", status: "<a href='http://nplus.doortodoor.co.kr/web/detail.jsp?slipno=587931883853' target='_blank' class='check-status' data-tracking='587931883853'>상태 확인</a>", myBid: "15,200,000원", review: "<a href='review.jsp?product_id=67890' class='btn btn-primary'>리뷰 작성</a>"}
            ];
        ...
        // 경매주문목록 링크 클릭 시 이벤트 처리
        document.getElementById('bidOrderListLink').addEventListener('click', function(event) {
            event.preventDefault();
            window.location.href = 'bidOrderList.jsp';
        });

        // 미결제목록 링크 클릭 시 이벤트 처리
        document.getElementById('outstandingListLink').addEventListener('click', function(event) {
            event.preventDefault();
            window.location.href = 'outstandingList.jsp';
        });
