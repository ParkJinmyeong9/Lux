    ...
    <div class="container content-box">
        ...
        <table class="table">
    		...
    		int pageSize = 10;
            	...
            	Vector<MyPageBean> orderList = auctionMgr.orderList(user_id);
            	// 전체 상품 개수를 가져옵니다.
        		int totalProducts = orderList.size();
            ...
            <tbody>
            	<% for (int i = startIdx; i < endIdx; i++) { 
            		MyPageBean bean = orderList.get(i);
            	%>
    			<tr>
      				...
      				<td>
					    ...
					    <% if (status.equals("배송 중")) { %>
					        <a href="http://nplus.doortodoor.co.kr/web/detail.jsp?slipno=587931883853">배송 중</a>
					    <% } else { %>
					        <%= status %>
					    <% } %>
					</td>   
      				<td><%= bean.getFinalPrice() %></td>
      				<td><button id="reviewBtn" class="btn btn-primary"><a href='review.jsp?product_id=<%=bean.getProductID() %>' class='btn btn-primary'>리뷰 작성</a></button></td> 
      			</tr>
  				<% } %>
    ...
    <script>
     ...
     // JavaScript 코드
        $(document).ready(function() {
            // 임시 데이터
            var bidData = [
                { number: 1, id: "122 (abcd1234)", product: "프라다 핑퐁 패들", status: "<a href='http://nplus.doortodoor.co.kr/web/detail.jsp?slipno=587931883853' target='_blank' class='check-status' data-tracking='587931883853'>상태 확인</a>", myBid: "2,900,000원", review: "<a href='review.jsp?product_id=12345' class='btn btn-primary'>리뷰 작성</a>"},
                { number: 2, id: "118 (efgh5678)", product: "프라다 리나일론 및 악어 가죽 스마트폰 케이스", status: "<a href='http://nplus.doortodoor.co.kr/web/detail.jsp?slipno=587931883853' target='_blank' class='check-status' data-tracking='587931883853'>상태 확인</a>", myBid: "6,600,000원", review: "<a href='review.jsp?product_id=67890' class='btn btn-primary'>리뷰 작성</a>"}
            ];
